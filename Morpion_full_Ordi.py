...
def jeu(grille:list[str], tour:str,j1:str,j2:str):
    ...
    col=random.randint(0,2) # pas besoin de controler
    lig=random.randint(0,2)
    print("")
    print(j2," a joué la case en position (", col, ",", lig, ")")
    time.sleep(2)
    ...
    while grille[int(col)+int(lig)*3]!=" ":
        ...
        col=random.randint(0,2) # pas besoin de controler
        lig=random.randint(0,2)
        print(j2," a joué la case (", col, ",", lig, ")")
        time.sleep(2)
...
def morpionFullPc(j1:str, j2:str, scorej1:int, scorej2:int)-> int:
    joueur : str
    gagne : int
    grille : list[str]
